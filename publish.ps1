...
# Write-Host is unavailable to callers that redirect or capture output, so status goes through the
# information stream instead. Colour is applied with $PSStyle so the console output is unchanged.
...
function Write-Status {
	...
	if ($Colour -and $PSStyle.OutputRendering -ne 'PlainText') {
		$Message = "$($PSStyle.Foreground.$Colour)$Message$($PSStyle.Reset)"
	}
